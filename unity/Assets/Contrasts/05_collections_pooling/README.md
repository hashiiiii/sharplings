...
|---|---|---|---|
| Collection expressions (`[10, 20, 30]`) and spread (`[.. baseHits, .. extraHits]`) | C# 12 | `expected — accepts ≤ 12 (Stage 1)` per `docs/feature-matrix.md`'s Stage 0 findings | A Stage 1 `csc.rsp` bump today, unofficially; official on Unity 6.8 |
...
|---|---|---|---|
| collection expression（`[10, 20, 30]`）と spread（`[.. baseHits, .. extraHits]`） | C# 12 | `docs/feature-matrix.md` の Stage 0 の結果によれば `expected — accepts ≤ 12 (Stage 1)` | 今日は非公式に Stage 1 の `csc.rsp` 引き上げ。Unity 6.8 では公式 |
