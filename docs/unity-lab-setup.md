...
### Concrete macOS plan for this machine (2026-07-12, executed — results below)
...
> **EN — Result (2026-07-12):** Steps 1–6 were executed as written, the same day. Step 1's facts re-verified unchanged. The backup lives at `~/Backups/unity-6000.7.0a2-dotnet-sdk-roslyn/Roslyn` (55 MB, 70 files, verified against the original). After the swap plus the side-by-side 10.0.9 runtime, the offline check printed `5.6.0-2.26270.133` and `-langversion:?` listed 13.0 and 14.0 (the new default). With `-define:SHARPLINGS_STAGE2` added to `Assets/Lab/csc.rsp`, the headless batchmode compile exited 0 with zero `error CS` lines, and the built `Sharplings.Lab.dll` demonstrably contains the C# 14 extension member (the probe's PASS branch is present, the SKIP branch absent) — Unity's Bee pipeline accepted the newer compiler, answering this section's open experimental question. The swap remains active; step 7's restore was deliberately not run. Rerun step 5 to detect the silent revert an editor update causes — if that happens while the define is still in `csc.rsp`, only the Lab assembly fails to compile until the swap is redone or the define line is dropped. Full evidence: `docs/feature-matrix.md`, "Stage 2 results".
>
> **JA — 実施結果 (2026-07-12):** 手順 1〜6 を、記載どおり同日に実行しました。手順 1 の事実は再確認の結果、変わっていませんでした。backup は `~/Backups/unity-6000.7.0a2-dotnet-sdk-roslyn/Roslyn` にあります（55 MB、70 ファイル、原本との一致を確認済み）。swap と 10.0.9 runtime の side-by-side 追加のあと、オフライン検証では `5.6.0-2.26270.133` が出力され、`-langversion:?` には 13.0 と 14.0（新しい default）が並びました。`Assets/Lab/csc.rsp` に `-define:SHARPLINGS_STAGE2` を追加した状態で headless batchmode compile は exit code 0・`error CS` ゼロ行で完走し、ビルドされた `Sharplings.Lab.dll` には C# 14 の extension member が実際に含まれています（probe の PASS 分岐が存在し、SKIP 分岐は不在）—— Unity の Bee pipeline は新しいコンパイラを受け入れ、この節の未解決だった実験課題に答えが出ました。swap は有効なままです。手順 7 の restore は意図的に実行していません。エディタ更新による静かな revert を検知するには手順 5 を再実行してください —— define が `csc.rsp` に残ったまま revert が起きた場合、swap をやり直すか define 行を消すまで、Lab assembly だけがコンパイルに失敗するようになります。証跡の全文は `docs/feature-matrix.md` の「Stage 2 results」を参照してください。
